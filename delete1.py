...
def sortingDone():
    ...
    for i in range(64):
        ...
        # To figure out if there is an open spot near.If 60% total avalible are this then spothappy (Weird Choice because not sure what names are already varibles)
        nearMe = 0 

        # To determine the % of spots that are satisified (because often there will be a point in which this is unsolveable)
        spotsHappy = 0 

        # First spot in upper left
        if i == 1:
            
            if fullList[i][1] != cyan:
            
                # Checking nearby spots 
                if fullList[i + 1][1] == fullList[i][1]:
                    nearMe = nearMe + 1 
                if fullList[i + 8][1] == fullList[i][1]:
                    nearMe = nearMe + 1 
                if fullList[i + 9][1] == fullList[i][1]:
                    nearMe = nearMe + 1 
                
                # Saying that this spot is sastified increasing the number of spots that are open
                if nearMe > 2:
                    spotsHappy = spotsHappy + 1
 
            
            elif fullList[i][1] == cyan:
                
                spotsHappy = spotsHappy + 1
                
        # Middle spots in top row
        if i > 1 and i < 8:
        	...
        	if fullList[i][1] != cyan:
        		...
        		if fullList[i + 1][1] == fullList[i][1]:
        			nearMe = nearMe + 1
        		if fullList[i + 7][1] == fullList[i][1]:
        			nearMe  = nearMe + 1 
        		if fullList[i + 8][1] == fullList[i][1]:
        			nearMe = nearMe + 1 
        		if fullList[i + 9][1] == fullList[i][1]:
